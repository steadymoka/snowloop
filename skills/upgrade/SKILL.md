...
1. `.tars/` 디렉토리 존재 확인. 없으면:
   > "tars이 초기화되지 않았습니다. `/tars:init`을 먼저 실행하세요."
2. 플러그인 버전 읽기: `${CLAUDE_SKILL_DIR}/../../.claude-plugin/plugin.json`의 `version`
3. 설치된 버전 읽기: `.tars/.version` (JSON)
   - `.version` 파일이 없으면 → version `"0.1.0"`, core를 `.tars/PROGRESS.md`의 `**Core**:` 값에서 추출
4. 버전 비교:
...
- `.tars/logs/backlog-archive/INDEX.md`
- `.tars/output/` 디렉토리 + `INDEX.md` (design core만)
- `.tars/MILESTONES.md`
- 향후 추가될 새 파일

#### 2b: COMMS.md 섹션 감지
- `.tars/COMMS.md`에 `### From Human (CLARIFY)` 섹션이 있는지 확인
- 없으면 추가 대상으로 표시

#### 2c: PROTOCOL.md 새 섹션 감지
1. `.tars/protocol/PROTOCOL.md`를 `## ` 기준으로 섹션 헤딩 추출
2. core별 템플릿 `${CLAUDE_SKILL_DIR}/../../cores/{core}/PROTOCOL.md`를 같은 방식으로 추출
3. 템플릿에 있지만 사용자 파일에 없는 `## ` 헤딩 = 추가 대상

#### 2d: .gitignore 감지
- `${CLAUDE_SKILL_DIR}/../../cores/shared/gitignore.snippet`의 각 줄이 `.gitignore`에 있는지 확인
...
#### 3a: 새 파일 생성
- 템플릿에서 읽어서 `.tars/`에 Write
- design core: output 디렉토리 구조 생성 (없는 경우)
...
- 템플릿에만 있는 새 섹션을 `.tars/protocol/PROTOCOL.md` 끝에 append
- 각 섹션 앞에 `---` 구분자 추가
- 기존 내용은 절대 수정하지 않음
- self-evolution이 다음 RETRO에서 정리함

#### 3d: .gitignore 갱신
- `# tars` 섹션이 있으면 해당 섹션에 누락 줄 추가
- 없으면 snippet 전체를 끝에 추가

#### 3e: .version 갱신
`.tars/.version`에 Write:
```json
{
  "version": "{new_version}",
  "core": "{core}",
