---
name: init
description: "Initialize autonomous agent protocol in current project. Scaffolds .snowloop/ directory with mode-specific templates (dev or design), creates heartbeat command, guard hook, and configures permissions."
...
```bash
mkdir -p .snowloop/protocol
mkdir -p .snowloop/logs/archive
mkdir -p .snowloop/logs/backlog-archive
```
...
**모드별 파일:**
- `${CLAUDE_SKILL_DIR}/../../templates/{mode}/MISSION.md` → `.snowloop/protocol/MISSION.md`
  - `{PROJECT_NAME}` → 사용자 입력값
  - `{MISSION_DESCRIPTION}` → 사용자 입력값
  - dev 모드: `{BUILD_CMD}`, `{TEST_CMD}`, `{LINT_CMD}` → 사용자 입력값
- `${CLAUDE_SKILL_DIR}/../../templates/{mode}/PROTOCOL.md` → `.snowloop/protocol/PROTOCOL.md`
...
- `${CLAUDE_SKILL_DIR}/../../templates/shared/PROGRESS.md` → `.snowloop/PROGRESS.md`
  - `{MODE}` → 선택한 모드 (dev 또는 design)
- `${CLAUDE_SKILL_DIR}/../../templates/shared/LOG.md` → `.snowloop/logs/LOG.md`
- `${CLAUDE_SKILL_DIR}/../../templates/shared/RETRO.md` → `.snowloop/logs/RETRO.md`
- `${CLAUDE_SKILL_DIR}/../../templates/shared/MILESTONES.md` → `.snowloop/MILESTONES.md`

**EVOLUTION.md** (`.snowloop/protocol/EVOLUTION.md`에 직접 생성):
```markdown
...
**backlog-archive/INDEX.md** (`.snowloop/logs/backlog-archive/INDEX.md`에 직접 생성):
```markdown
...
```markdown
.snowloop/protocol/PROTOCOL.md를 읽고 Heartbeat Cycle을 정확히 실행하세요.
...
### 다음 단계
1. `.snowloop/protocol/MISSION.md`를 프로젝트에 맞게 커스터마이징
