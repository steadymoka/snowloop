...
### 6. LOG 아카이브
- LOG.md 항목이 50개 초과 시 → 오래된 항목을 `.snowloop/logs/log-{date}.md`로 이동
...
- **Declining productivity**: 3개 연속 RETRO에서 productive_ratio 하락 → COMMS에 `[→human]` 전략 재검토 제안

### Oscillation Detection
- EVOLUTION.md에서 같은 영역의 patch→revert 가 2회 반복 → `[LATERAL-THINK]` 태그로 구조적 재설계 제안

### Regression Detection
- 직전 protocol patch 적용 후 메트릭이 악화된 항목 식별
- 악화 확인 시: patch를 PROTOCOL.md에서 되돌리고, EVOLUTION.md에 REVERTED 기록
